						  ...
						  const XMMATRIX& viewMatrix, const XMMATRIX& orthoMatrix)
{
	...
	result = m_MiniMapBitmap->Render(deviceContext, m_mapLocationX, m_mapLocationY,154,154);
	if(!result)
	{
		return false;
	}
	...
	result = m_PointBitmap->Render(deviceContext, m_pointLocationX, m_pointLocationY,3,3);
	if(!result)
	{
		return false;
	}
